...
// Bugs
// The turn technically ends, but the loop starts so qiuckly that its immediately the same player's turn
	...
	switch(in){
		...
		case 'f':
			...
			if (players[opposite_turn].map[players[turn].y][players[turn].x] == SHIP) {
				...
				fprintf(screen[turn], "\033[%d;%dH", 24, 1);
				//fprintf(screen[turn], "You've already fired there!!!!");
...
void battleSetup() {
	...
	while (1) {
		if (player1done == 1 && player2done == 1) {
			...
			//change semaphore type
			semaphore[0].count = 17;
			semaphore[1].count = 17;
...
void player1() {
	...
	while(1) {
		...
		if (stage == 1){
			...
			fprintf(com0out, "\033[%d;%dH", 24, 1);
			fprintf(com0out, "Player 1's turn");
			...
			//while(!players[1].fired) {
			//	fprintf(com0out, "\033[%d;%dH", 24, 1);
			//	fprintf(com0out, "Player 2's turn");
			//	fflush(com0out);
			//}
...
void player2() {
	...
	while(1) {
		if (stage == 0 && player2done == 0) {
			...
			player2done = 1;
			fprintf(com1out, "Flag set! player2done = %d");
			fflush(com1out);
		...
		if (stage == 1){
			...
			fprintf(com1out, "\033[%d;%dH", 24, 1);
			fprintf(com1out, "Player 2's turn");
			...
			fprintf(com1out, "\033[%d;%dH", 25, 1);
			fprintf(com1out, "Player fired!");
			...
			//while(!players[0].fired) {
			//	fprintf(com1out, "\033[%d;%dH", 24, 1);
			//	fprintf(com1out, "Player 1's turn");
			//	fflush(com1out);
			//}
