...
****************************************************************
**  GET_INPUT
**  detects several special key for stopwatch input
**	author: Rafii
****************************************************************
GET_INPUT:
	movem.l	%d5/%a5, -(%sp)

	cmpi.b	#0, %d0			| skip if input size is 0
	beq	GET_INPUT_END
	
	lea.l	BUF, %a5		| get input from buffer
	move.b	(%a5), %d5

	cmp.b	#'s', %d5		| check input
	beq	START_STOPWATCH
	cmp.b	#'t', %d5
	beq	TERMINATE_STOPWATCH
	cmp.b	#'r', %d5
	beq	RESET_STOPWATCH
	bra	GET_INPUT_END
	
START_STOPWATCH:
	** ignore command if timer is up
	cmpi.b	#1, STATUS
	beq	GET_INPUT_END
...
TERMINATE_STOPWATCH:
	** ignore command if timer is down
	cmpi.b	#0, STATUS
	beq	GET_INPUT_END
...
STOPWATCH_INIT:	
	** test display
	...
	/* stopwatch initiated at 23:58:55 for testing purpose */
	** initiate time var
	move.b	#0x05, SECOND_1
	move.b	#0x05, SECOND_2
	move.b	#0x08, MINUTE_1
	move.b	#0x05, MINUTE_2
	move.b	#0x03, HOUR_1
	move.b	#0x02, HOUR_2
	
	** Start up SET_TIMER by the system call
	move.l	#SYSCALL_NUM_SET_TIMER, %d0
	move.w	#10000, %d1			/* change timer (in 0.t msec) */
	move.l	#STOPWATCH_INCREASE_TIME, %d2	/* change '#TT' to the function label you want to use */
	trap	#0
	
	** print something
	move.l	#SYSCALL_NUM_PUTSTRING, %d0
	move.l	#0, %d1				/*ch = 0*/
	move.l	#W_MSG, %d2			/*p = #TMSG*/
	move.l	#12, %d3			/*size = 12*/
	trap	#0
...
STOPWATCH_DISPLAY_TIME:
	...
	move.b	SECOND_1, %d0			| move SECOND_1 to d0
	andi.b	#0x0F, %d0			| get last digit
	addi.b	#48, %d0			| add header
	move.b	%d0, LED0			| display led
	
	move.b	SECOND_2, %d0			| move SECOND_2 to d0
	andi.b	#0x0F, %d0			| get last digit
	addi.b	#48, %d0			| add header
	move.b	%d0, LED1			| display led
	
	move.b	#':', LED2			| display led
	
	move.b	MINUTE_1, %d0			| move MINUTE_1 to d0
	andi.b	#0x0F, %d0			| get last digit
	addi.b	#48, %d0			| add header
	move.b	%d0, LED3			| display led
	
	move.b	MINUTE_2, %d0			| move MINUTE_2 to d0
	andi.b	#0x0F, %d0			| get last digit
	addi.b	#48, %d0			| add header
	move.b	%d0, LED4			| display led
	
	move.b	#':', LED5			| display led
	
	move.b	HOUR_1, %d0			| move HOUR_1 to d0
	andi.b	#0x0F, %d0			| get last digit
	addi.b	#48, %d0			| add header
	move.b	%d0, LED6			| display led
	
	move.b	HOUR_2, %d0			| move HOUR_2 to d0
	andi.b	#0x0F, %d0			| get last digit
	addi.b	#48, %d0			| add header
	move.b	%d0, LED7			| display led
	
	movem.l	(%sp)+, %d0
	rts

STOPWATCH_INCREASE_TIME:
    	** skip if stopwatch down
    	cmpi.b	#1, STATUS
	bne	TIME_INCREMENT_END
	
	** increment time per digit
	INCREMENT_SECOND_1:
	addi.b	#1, SECOND_1			| increment second_1 by 1
	cmpi.b	#0x0a, SECOND_1			| check second_1 digit
	blt	TIME_INCREMENT_END		| terminate if <10
	
	move.b	#0x00, SECOND_1			| reset second_1 digit
	
	INCREMENT_SECOND_2:
	addi.b	#1, SECOND_2			| increment second_2 by 1
	cmpi.b	#0x06, SECOND_2			| check second_2 digit
	blt	TIME_INCREMENT_END		| terminate if <6
	
	move.b	#0x00, SECOND_1			| reset second_1 digit
	move.b	#0x00, SECOND_2			| reset second_2 digit
	
	INCREMENT_MINUTE_1:
	addi.b	#1, MINUTE_1			| increment MINUTE_1 digit by 1
	cmpi.b	#0x0a, MINUTE_1			| check MINUTE_1 digit
	blt	TIME_INCREMENT_END		| terminate if <10
	
	move.b	#0x00, SECOND_1			| reset second_1 digit
	move.b	#0x00, SECOND_2			| reset second_2 digit
	move.b	#0x00, MINUTE_1			| reset minute_1 digit
	
	INCREMENT_MINUTE_2:
	addi.b	#1, MINUTE_2			| increment MINUTE_2 digit
	cmpi.b	#0x06, MINUTE_2			| check MINUTE_2 digit
	blt	TIME_INCREMENT_END		| terminate if <6
	
	move.b	#0x00, SECOND_1			| reset second_1 digit
	move.b	#0x00, SECOND_2			| reset second_2 digit
	move.b	#0x00, MINUTE_1			| reset minute_1 digit
	move.b	#0x00, MINUTE_2			| reset minute_2 digit
	
	INCREMENT_HOUR_1:
	addi.b	#1, HOUR_1			| increment hour_2 digit
	bra	CHECK_24H_LIMIT			| check 24h limit
	CHECK_24H_LIMIT_END:
	cmpi.b	#0x0a, HOUR_1			| check MINUTE_1 digit
	blt	TIME_INCREMENT_END		| terminate if <6
	...
	move.b	#0x00, MINUTE_1			| reset minute_1 digit
	move.b	#0x00, MINUTE_2			| reset minute_2 digit
	move.b	#0x00, HOUR_1			| reset hour_1 digit
	
	INCREMENT_HOUR_2:
	addi.b	#1, HOUR_2			| increment hour_2 digit
	
	TIME_INCREMENT_END:
	rts
	
	CHECK_24H_LIMIT:			| check whether it has reached 24h or not
	cmpi.b	#0x04, HOUR_1
	bne	TIME_INCREMENT_END
	cmpi.b	#0x02, HOUR_2
	bne	TIME_INCREMENT_END
	...
	move.b	#0x00, MINUTE_2			| reset minute_2 digit
	move.b	#0x00, HOUR_1			| reset hour_1 digit
	move.b	#0x00, HOUR_2			| reset hour_2 digit
	
	bra	CHECK_24H_LIMIT_END

STOPWATCH_ON:
	** set status to timer up
	move.b	#1, STATUS
	...
	** print something
	move.l	#SYSCALL_NUM_PUTSTRING, %d0
	move.l	#0, %d1				/*ch = 0*/
	move.l	#S_MSG, %d2			/*p = #TMSG*/
	move.l	#22, %d3			/*size = 22*/
	trap	#0
...
STOPWATCH_RESET:
	move.b	#0x00, SECOND_1
	move.b	#0x00, SECOND_2
	move.b	#0x00, MINUTE_1
	move.b	#0x00, MINUTE_2
	move.b	#0x00, HOUR_1
	move.b	#0x00, HOUR_2
	...
	** print something
	move.l	#SYSCALL_NUM_PUTSTRING, %d0
	move.l	#0, %d1				/*ch = 0*/
	move.l	#R_MSG, %d2			/*p = #TMSG*/
	move.l	#20, %d3			/*size = 20*/
	trap	#0
...
****************************************************************
**	Data region with an initial value for stopwatch
****************************************************************
.section .data
STATUS:		.dc.b	0
		.even
...
HOUR_2:		.dc.b	0
		.even
W_MSG:		.ascii	"Hi, Welcome!"			| size=12
            .even
S_MSG:		.ascii	"\r\nStopwatch started!\r\n"	| size=22
            .even
T_MSG:		.ascii	"\r\nStopwatch stopped!\r\n"	| size=22
