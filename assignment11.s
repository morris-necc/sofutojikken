...
** Let user input the character screen
** Don't output a failed input to screen
** Change time limit depending on thelength of a character string
** Count the numbers of correct & failed inputs and display each
** Calculate the typing speed (characters per minute) & display it
** (Optional) 
** Format the time that has elapsed since program execution started
** Make the character display speed change depending on the chosen level
************************************* 
...
WAIT:
	...
	move.l	#SYSCALL_NUM_SET_TIMER, %d0
	move.l	#1200000, %d1			/* 120 seconds */
	move.l	#WAIT_END, %d2			/* Finish waiting*/	
	trap	#0
...
WAIT_LOOP:
	...
	cmpi.b	#0, CONFIRMATION_BUFFER
	beq	WAIT_LOOP

	move.b	CONFIRMATION_BUFFER, %d5
...
MODEASK1:	.ascii	"1 - Word Mode\r\n"
MODEASK2:	.ascii	"2 - Paragraph Mode\r\n"
